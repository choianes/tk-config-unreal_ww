...
class MayaSessionGeometryPublishPlugin(HookBaseClass):
    ...
    def publish(self, settings, item):
        ...
        # Replace Alembic logic with FBX export
        try:
            # Use cmds.file to export the selection as an FBX file
            self.logger.info(f"Exporting FBX to {publish_path}")
            cmds.file(
            ...
            self.logger.info(f"FBX export complete: {publish_path}")

        except Exception as e:
            self.logger.error(f"Failed to export FBX: {e}")
            raise
